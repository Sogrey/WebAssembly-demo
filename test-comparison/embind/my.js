 ...
 * @version v1.0.0
 */
LibraryCadWasm = (function () {

    var _that;

    function _(api, options) {
        this._API = api;
        if (this._API)
            this._API.wasm = {};
        this.wasmPaths = options.wasmPaths && (Array.isArray(options.wasmPaths) ? options.wasmPaths : [options.wasmPaths]) || ['./embind.wasm'];
        ...
        return new Promise(async function (resolve, reject) {
            try {
                ...
                if (ENVIRONMENT_IS_NODE) {
                    if (!(typeof process == 'object' && typeof require == 'function')) throw new Error(
                    ...
                    if (ENVIRONMENT_IS_SHELL) {
                        ...
                        if ((typeof process == 'object' && typeof require === 'function') || typeof window == 'object' ||
                            typeof importScripts == 'function') throw new Error(
                                'not compiled for this environment (did you build to HTML and try to run it not on the web, or set ENVIRONMENT to something - like node - and run it someplace else - like on the web?)'
                            );
                                ...
                                readAsync = (url, onload, onerror) => {
                                    ...
                                    xhr.onload = () => {
                                        if (xhr.status == 200 || (xhr.status == 0 && xhr
                                            .response)) { // file URLs can return 0
                                            onload(xhr.response);
                                            return;
                                        }
                                        onerror();
                ...
                function UTF8ArrayToString(heapOrArray, idx, maxBytesToRead) {
                    ...
                    while (idx < endPtr) {
                        ...
                        if ((u0 & 0xF0) == 0xE0) {
                            ...
                            if ((u0 & 0xF8) != 0xF0) warnOnce('Invalid UTF-8 leading byte 0x' + u0.toString(16) +
                ...
                function stringToUTF8Array(str, heap, outIdx, maxBytesToWrite) {
                    if (!(maxBytesToWrite >
                        0)) // Parameter maxBytesToWrite is not optional. Negative values, 0, null, undefined and false each don't write out any bytes.
                        return 0;
                    ...
                    for (var i = 0; i < str.length; ++i) {
                        ...
                        if (u <= 0x7F) {
                            ...
                            if (u > 0x10FFFF) warnOnce('Invalid Unicode code point 0x' + u.toString(16) +
                ...
                if (Module['TOTAL_STACK']) assert(TOTAL_STACK === Module['TOTAL_STACK'],
                    'the stack size can no longer be determined at runtime')

                var INITIAL_MEMORY = Module['INITIAL_MEMORY'] || 16777216;
                legacyModuleProp('INITIAL_MEMORY', 'INITIAL_MEMORY');

                assert(INITIAL_MEMORY >= TOTAL_STACK, 'INITIAL_MEMORY should be larger than TOTAL_STACK, was ' +
                ...
                function checkStackCookie() {
                    ...
                    if (cookie1 != 0x2135467 || cookie2 != 0x89BACDFE) {
                        abort('Stack overflow! Stack cookie has been overwritten at 0x' + max.toString(16) +
                            ', expected hex dwords 0x89BACDFE and 0x2135467, but received 0x' + cookie2.toString(16) +
                            ' 0x' + cookie1.toString(16));
                    }
                    // Also test the global address 0 for integrity.
                    if (HEAPU32[0] !== 0x63736d65 /* 'emsc' */) abort(
                ...
                var dependenciesFulfilled =
                    null; // overridden to take different actions when all run dependencies are fulfilled
                    ...
                    function instantiateArrayBuffer(receiver) {
                        return getBinaryPromise().then(function (binary) {
                            ...
                            if (isFileURI(wasmBinaryFile)) {
                                err('warning: Loading from a file URI (' + wasmBinaryFile +
                ...
                function setValue(ptr, value, type = 'i8') {
                    ...
                    switch (type) {
                        ...
                        case 'i32':
                            ...
                            break;
                        case 'i64':
                            (tempI64 = [value >>> 0, (tempDouble = value, (+(Math.abs(tempDouble))) >= 1.0 ? (tempDouble > 0.0 ?
                ...
                function new_(constructor, argumentList) {
                    if (!(constructor instanceof Function)) {
                        throw new TypeError('new_ called with constructor type ' + typeof (constructor) +
                ...
                function craftInvokerFunction(humanName, argTypes, classType, cppInvokerFunc, cppTargetFunc) {
                    ...
                    for (var i = 1; i < argTypes.length; ++i) { // Skip return value at index 0 - it's not deleted here.
                        if (argTypes[i] !== null && argTypes[i].destructorFunction ===
                            undefined) { // The type does not define a destructor function - must use dynamic stack
                    ...
                    if (needsDestructorStack) {
                        ...
                        for (var i = isClassMethodFunc ? 1 : 2; i < argTypes.length; ++
                            i) { // Skip return value at index 0 - it's not deleted here. Also skip class type if not a method.
                        ...
                        proto[methodName] = function () {
                            ...
                            if (!proto[methodName].overloadTable.hasOwnProperty(arguments.length)) {
                                throwBindingError("Function '" + humanName +
                                    ...
                                    ") - expects one of (" + proto[methodName].overloadTable + ")!");
                            }
                ...
                function exposePublicSymbol(name, value, numArguments) {
                    ...
                    if (Module.hasOwnProperty(name)) {
                        if (undefined === numArguments || (undefined !== Module[name].overloadTable && undefined !== Module[
                            name].overloadTable[numArguments])) {
                ...
                function __embind_register_function(name, argCount, rawArgTypesAddr, signature, rawInvoker, fn) {
                    ...
                    whenDependentTypesAreResolved([], argTypes, function (argTypes) {
                        ...
                        replacePublicSymbol(name, craftInvokerFunction(name, invokerArgsArray,
                            null /* no class 'this'*/, rawInvoker, fn), argCount - 1);
                        return [];
                    });
                }
                ...
                function __embind_register_integer(primitiveType, name, size, minRange, maxRange) {
                    ...
                    var checkAssertions = (value, toTypeName) => {
                        ...
                        if (value < minRange || value > maxRange) {
                            throw new TypeError('Passing a number "' + embindRepr(value) +
                ...
                var _emscripten_stack_init = Module["_emscripten_stack_init"] = function () {
                    return (_emscripten_stack_init = Module["_emscripten_stack_init"] = Module["asm"][
                ...
                var _emscripten_stack_get_free = Module["_emscripten_stack_get_free"] = function () {
                    return (_emscripten_stack_get_free = Module["_emscripten_stack_get_free"] = Module["asm"][
                ...
                var _emscripten_stack_get_base = Module["_emscripten_stack_get_base"] = function () {
                    return (_emscripten_stack_get_base = Module["_emscripten_stack_get_base"] = Module["asm"][
                ...
                var _emscripten_stack_get_end = Module["_emscripten_stack_get_end"] = function () {
                    return (_emscripten_stack_get_end = Module["_emscripten_stack_get_end"] = Module["asm"][
    ...
    _.prototype.init = function () {
        const self = this;
        if (self.wasmPaths.length > 0) {
            for (let index = 0; index < self.wasmPaths.length; index++) {
                const wasmUrl = self.wasmPaths[index];
                getExportFunctionPromise(wasmUrl)
                    .then(result => {

                        console.log(result);

                        if (self._API)  // 将c函数附加给javascript对象
                            self._API.wasm = result;

                        // if (result) {
                        //     if (result.exports) {
                        //         for (let func in result.exports) {
                        //             // console.log(func);
                        //             if (self._API)  // 将c函数附加给javascript对象
                        //                 self._API.wasm[func] = result.exports[func];
                        //         }
                        //     }
                        // }
